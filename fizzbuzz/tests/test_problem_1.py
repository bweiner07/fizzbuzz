...
class TestProblem1(unittest.TestCase):
    def setUp(self):
        ...
        self.events = [
            ...
            IgnitionOff(event_start_time + timedelta(hours=1), skid_loader),
            IgnitionOff(event_start_time + timedelta(hours=3), boom_lift),
            IgnitionOn(event_start_time + timedelta(days=2), skid_loader),
            IgnitionOn(event_start_time + timedelta(days=2), boom_lift),
            IgnitionOff(event_start_time + timedelta(days=2, hours=3), skid_loader),
        ]

    def test_engine_status_is_stored_correctly_when_events_are_in_order(self):
